...
func (h *Handler) createOrder(ps []types.Product, items []types.CartItem, userID int) (int, float64, error) {
	...
	//calculate th total price
	totalPrice := calculateTotalPrice(items, productMap)

	//reduce the quantity of product from db
...
func checkIfCartIsInStock(cartItems []types.CartItem, products map[int]types.Product) error {
	...
	for _, item := range cartItems {
		...
		if !ok {
			return fmt.Errorf("product %d is not available in cart,pleased refresh your cart", item.ProductID)
		}
		if product.Quantity < item.Quantity {
			return fmt.Errorf("product %s is not avilable in quantity requested", product.Name)
		}
	}
	return nil
}
